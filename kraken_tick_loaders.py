...
async def on_message(websocket, message):
    try:
        ...
        trades = data['data']
        for trade in trades:
            date = datetime.fromisoformat(trade['timestamp'].replace("Z", '')) # "timestamp":"2024-09-05T21:35:00.345585Z"
            ticker = trade['symbol']
            price = trade['price']
            quantity = trade['qty']
        ...
        if (not ("subscribe" in data.get("method", "error")) and (not (data == {"channel":"heartbeat"}))):

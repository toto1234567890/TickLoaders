...
async def on_message(websocket, message):
    try:
        ...
        ticker = data['product_id'].replace('-', '')
        price = data['price']
        quantity = data['size']
        ...
        if not (data.get("type", False) == "subscriptions"):
...
async def subscribe(websocket, ticker):
    subscribe_message = {"type": "subscribe",
                         "product_ids": [f"{ticker.upper().replace('/', '-')}"],
                         "channels" : ["matches"]}
    try:
...
async def watch_broker_ticks(name, logger):
    ...
    async with websockets.connect(uri) as websocket:
        ...
        subscribe_message = {"type": "subscribe",
                             "product_ids": [f"{ticker.upper().replace('/', '-')}" for ticker in tickerList],
                             "channels" : ["matches"]}
        await websocket.send(jsonDumps(subscribe_message))

...
requested_token_url = "https://api.kucoin.com/api/v1/bullet-public"
...
async def on_message(websocket, message):
    try:
        ...
        trade = data["data"]
        microseconds = float(trade['time'])
        seconds, microseconds = divmod(microseconds, 10**9)
        date = datetime.utcfromtimestamp(seconds) + timedelta(microseconds=microseconds)
        ticker = trade['symbol']
        price = trade['price']
        quantity = trade['size']
...
async def subscribe(websocket, ticker):
    ...
    maxId = ids["maxId"]+1
    subscribe_message = {"id": maxId, 
                        "type": "subscribe", 
                        "topic": f'/market/match:{ticker}', 
                        "privateChannel": False, 
                        "response": True}
    try:
        await websocket.send(jsonDumps(subscribe_message))
        ids[ticker] = maxId ; ids["maxId"] = maxId
    except Exception as e:
...
async def watch_broker_ticks(name, logger):
    ...
    async with websockets.connect(uri) as websocket:
        ...
        for ticker in tickerList : 
            id+=1 ; ids[ticker.replace("/", "-")]=id ; ids["maxId"]=id
            await websocket.send(jsonDumps({"id": id, 
                                            "type": "subscribe", 
                                            "topic": f'/market/match:{ticker.replace("/", "-")}', 
                                            "privateChannel": False, 
                                            "response": True}))
...
async def check_tickers_list(name, section):
    ...
    try:
        try:
            ...
            new_tickerList = [ticker.replace("/", "-") for ticker in new_tickerList]
...
# kucoin specific
def kucoin_request_token(get_token_url):
    from requests import post as requestsPost
    token = (jsonLoads(requestsPost(get_token_url).text)).get('data').get('token')
    return token
ids={"maxId":0}
...
if __name__ == "__main__":
    ...
    uri += f"?token={kucoin_request_token(requested_token_url)}"

...
BROKER = 'finnhub'
uri = "wss://ws.finnhub.io?token=c2vs952ad3ifkigc0kng"

# c in trade data :
# 1  ->     Regular     : A standard trade executed during normal market hours
# 2  ->     Acquisition : Trade as part of an acquisition
# 3  ->     A trade where the price is based on an average of multiple transactions
# 4  ->     A trade where multiple orders were combined into a single execution
# 5  ->     A trade that settles the same day (order can be executed long time after the placing)
# 6  ->     Distribution trade, profit of asset hold re-invested in same asset (example with ETF)
# 7  ->     
# 12 ->     Odd lot : std lot size = 100, trade of incomplete lot
# 24 ->     Trade executed outside of the stock market hours
...
async def on_message(websocket, message):
    ...
    try:
        ...
        for trade in data['data']:
            date = datetime.utcfromtimestamp(trade['t']/1000)
            ticker = trade['s']
            price = trade['p']
            quantity = trade['v']
            tradeID = 0
            last_asset_price = last_price.get(ticker, 0)
            if last_asset_price > price:      ordered = "sell"
            elif last_asset_price < price:    ordered = "buy"
            else:                       ordered = "unknow" 
            last_price[ticker] = price 
            await async_post(ticker=ticker, record=[date, standardStr(ticker), price, quantity, tradeID, ordered])
    except Exception as e: 
        if not (data == {'type':'ping'} ): # subscribe, unsubscribe response...
...
async def subscribe(websocket, ticker):
    subscribe_message = f'{{ "type": "subscribe", "symbol": "{ticker}" }}'
    try:
...
async def unSubscribe(websocket, ticker):
    global dataList
    unsubscribe_message = f'{{ "type": "unsubscribe", "symbol": "{ticker}" }}'
    try:
        await websocket.send(unsubscribe_message)
    except Exception as e:
...
def run_tick_loader(name, host="127.0.0.1", conf="analyst", mem_section=False, log_level=LOG_LEVEL, mode=DEFAULT_MODE, tick_exposer=False, db_file_name=False, link_to_analyst=False):
    ...
    if mode == "OneBrokerFull".lower():
        ...
        if (config.mem_config == None) or (not config.mem_config.get(mem_section, False)) or (not config.mem_config[mem_section].get(BROKER, False)):
            config.update_mem_config(section_key_val_dict={mem_section:{BROKER:'NVDA'}})
        ...
        if not mem_section:
            mem_section = name.replace(BROKER+"_", '').lower().replace(".py", '')
        if (config.mem_config == None) or (not config.mem_config.get(mem_section, False)) or (not config.mem_config[mem_section].get(BROKER, False)):
            config.update_mem_config(section_key_val_dict={mem_section:{BROKER:'MSTFT'}})

...
async def on_message(websocket, message):
    try:
        ...
        trade = data["data"]
        microseconds = float(trade['microtimestamp'])
        seconds, microseconds = divmod(microseconds, 10**6)
        date = datetime.utcfromtimestamp(seconds) + timedelta(microseconds=microseconds)
        ticker = (data['channel']).replace("live_trades_", '')
        price = trade['price']
        quantity = trade['amount']
        tradeID = trade['id']
        ordered = "sell" if bool(trade['type']) else "buy"
        await async_post(ticker=ticker, record=[date, standardStr(ticker), price, quantity, tradeID, ordered])
    except Exception as e: 
        if not "subscription_succeeded" in data["event"]: # for subscribe and unsubscribe msg
            if (data["event"] == 'bts:request_reconnect'): 
                # raise broker error, reconnect anyway...
                await websocket.logger.asyncError("{0} : broker error, bitstamp request reconnection : {1}".format(name, str(data)))
            else:
...
async def subscribe(websocket, ticker):
    subscribe_message = {"event": "bts:subscribe",
                         "data": {
                            "channel": f"live_trades_{standardStr(ticker)}"}
                        }
    try:
...
async def unSubscribe(websocket, ticker):
    global dataList
    unsubscribe_message = {"event": "bts:unsubscribe",
                           "data": {
                               "channel": f"live_trades_{standardStr(ticker)}"}
                          }
    try:

...
async def on_message(websocket, message):
    try:
        ...
        if data['action'] == "update": 
            for trade in trades:
                date = datetime.utcfromtimestamp(int(trade['ts'])/1000)
        ...
        if not ("subscribe" in data.get("event", "error")):
...
async def subscribe(websocket, ticker):
    subscribe_message = {"op": "subscribe",
                           "args": [{"instType":"SPOT", "channel": "trade", "instId":"{0}".format(ticker.replace('/', '').upper())}]}
    try:
...
async def unSubscribe(websocket, ticker):
    global dataList
    unsubscribe_message = {"op": "unsubscribe",
                           "args": [{"instType":"SPOT", "channel": "trade", "instId":"{0}".format(ticker.replace('/', '').upper())}]}
    try:
...
async def watch_broker_ticks(name, logger):
    ...
    async with websockets.connect(uri) as websocket:
        ...
        for ticker in tickerList:
            subscribe_list={"instType":"SPOT", "channel": "trade", "instId":"{0}".format(ticker.replace('/', '').upper()), "action": "update"}
        subscribe_message = {"op": "subscribe",
                             "args": [subscribe_list]}
        await websocket.send(jsonDumps(subscribe_message))
